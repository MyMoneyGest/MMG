...
export const sendConfirmationCode = functions.https.onCall(async (data, context) => {
  if (!data || typeof data.email !== 'string') {
    throw new functions.https.HttpsError('invalid-argument', 'Email est requis et doit être une chaîne de caractères');
  }
  ...
  // Anti spam : on vérifie si un code a été créé dans les 5 dernières minutes
  if (doc.exists) {
    const data = doc.data();
    const createdAt = data?.createdAt?.toDate?.();
    if (createdAt && (Date.now() - createdAt.getTime()) < 5 * 60 * 1000) {
      throw new functions.https.HttpsError('resource-exhausted', 'Un code a déjà été envoyé récemment. Veuillez patienter.');
    }
  }
  ...
  const msg = {
    to: email,
    from: 'mymoneygest@gmail.com', // à changer
