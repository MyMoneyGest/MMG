// MoovMoneyEntrepriseDetailsScreen.tsx
...
type Props = NativeStackScreenProps<RootStackParamList, 'MoovMoneyEntrepriseDetailsScreen'>;

const MoovMoneyEntrepriseDetailsScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const ref = doc(db, 'companies', companyId, 'linkedAccounts', 'moov');
    const unsubscribe = onSnapshot(ref, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.data();
        setBalance(data?.moovBalance ?? 0);
        setTransactions(
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Moov Money - Solde</Text>
...
export default MoovMoneyEntrepriseDetailsScreen;
